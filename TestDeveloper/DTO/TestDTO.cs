...
namespace TestDeveloper.API.DTO
{
    public class KnowledgeTestDTO
    {
        ...
        public KnowledgeTest ToKnowldgeTest()
        {
            KnowledgeTest knowledgeTest = new KnowledgeTest();
            knowledgeTest.Id = Id;
            knowledgeTest.Title = Name;
            knowledgeTest.Description = Description;
            knowledgeTest.Option = new Option();
            knowledgeTest.Option.TestTime = OptionDTO.TestTime;
            knowledgeTest.Option.NumberOfEttemps = OptionDTO.NumberOfEttemps;
            knowledgeTest.Option.NumberOfQuestions = OptionDTO.NumberOfQuestions;
            foreach (var item in QuestionDTOs)
            {
                Type t = item.GetType();
                if (t.Equals(typeof(SingleCaseQuestion)))
                {
                    knowledgeTest.Questions.Add(item.ToQuestion() as SingleCaseQuestion);
                }
                else if (t.Equals(typeof(MultipleCaseQuestion)))
                {
                    knowledgeTest.Questions.Add(item.ToQuestion() as MultipleCaseQuestion);
                }
            }
